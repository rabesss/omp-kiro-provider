...
- `models.json` overlay for reviewed capabilities and for cold-start or unauthenticated fallback.
...
`models.json` is the capability overlay and the cold-start or unauthenticated fallback. It records context windows, max-token limits, reasoning flags, and text or image capability flags. Discovery requires auth. There is no public catalog. If discovery fails, the session is offline, or you are unauthenticated, the provider uses the static `models.json` catalog.
...
The provider registers the overlay at startup. When OMP passes a credential to `fetchDynamicModels`, the provider merges live `ListAvailableModels` ids with that overlay. When reviewed capability metadata changes, update `models.json` in a reviewable PR and run the test suite before merging.
